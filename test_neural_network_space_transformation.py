...
X, y = data_generator.generate_parabolic_data(N, num_dims, num_classes, seed=0)
...
neural_net.train(X, y, print_loss=100)
...
separating_plane = (Ws2 * Hs)[0, 0]
separating_plane_plot = [region_plot(separating_plane > 0, (xs, -1, 1),
                                     (ys, -1, 1), incol=bg_colors[0]),
                         region_plot(separating_plane < 0, (xs, -1, 1),
                                     (ys, -1, 1), incol=bg_colors[1])]
...
transformed_separating_plane = (Ws2 * Xs)[0, 0]
...
for i in range(len(transformed_boundary_planes)):
    if i < len(transformed_boundary_planes) / 2:
        transformed_separating_plane_plots.append(region_plot(
            [analytic_planes[i] < ys, transformed_separating_plane > 0],
            xy_limits[i][0], xy_limits[i][1], incol=bg_colors[0]))
        transformed_separating_plane_plots.append(region_plot(
            [analytic_planes[i] < ys, transformed_separating_plane < 0],
            xy_limits[i][0], xy_limits[i][1], incol=bg_colors[1]))
    else:
        transformed_separating_plane_plots.append(region_plot(
            [analytic_planes[i] > ys, transformed_separating_plane > 0],
            xy_limits[i][0], xy_limits[i][1], incol=bg_colors[0]))
